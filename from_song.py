...
def app():
    ...
    if "code" in st.experimental_get_query_params():
        ...
        if token_info:
            ...
            tracks_to_add = ["spotify:track:" + id for id in recommended_playlist["id"]]
            ...
            st.toast(
                "Authentication completed successfully, Playlist has been added to user Library",
                icon="✅",
            )
        ...
        if search_query:
            ...
            with col1:
                selected_song_index = st.selectbox(
                    "AVAILABLE SONGS",
                    range(len(suggestions)),
                    format_func=lambda x: display_suggestions[x],
                )
            with col2:
                number_of_songs = int(
                    st.number_input("NUMBER OF SONGS", min_value=3, max_value=15)
                )
            with col3:
            ...
            if search_but:
                with m_col1:
                    ...
                    st.markdown(
                        "<h3 style='text-align:center; color:#1cbc55'>RECOMMENDATIONS</h3>",
                        unsafe_allow_html=True,
                    )
                    ...
                    st.link_button(
                        label="ADD TO PLAYLIST", url=auth_url, use_container_width=True
                    )
