...
if "code" in st.experimental_get_query_params():
    ...
    if token_info:
        ...
        playlist_description = f'The following songs were generated by the Spotify Recommendation Engine on {str(datetime.now())}, designed by AyushMaan.'
        user_id = sp1.me()['id']
            ...
            <div style="background-color: #121313; padding:30px; margin-bottom:20px; border-radius: 50px; ">
                <div>
                    ...
                    <div style="display: inline-block; vertical-align: top;">
                        <h4 style='color:#1cbc55'>Hello {sp1.me()['display_name']}, </h4>
                        <h5>Here's your new Spotify playlist</h5>
                    </div>
                </div>
                <h5 style='margin-top:50px; text-align:center'>{playlist['name']}</h5>
                <p style='text-align:center>{playlist['description']}</p>
                <ul>
            """
            for index, row in recommended_playlist.iterrows():
                playlist_card = playlist_card + \
                    f"<li><a style='color:#1cbc55' href='{row['preview_url']}'>{row['name']}</a> - {', '.join(list(ast.literal_eval(row['artists'])))}</li>"
            playlist_card = playlist_card + \
                "</ul></div><div style='text-align:center'><a href='/' target='_self'><button style='background-color: transparent; border: 2px solid #1cbc55; color: #1cbc55; padding: 10px 20px; cursor: pointer;'>GO BACK TO RECOMMENDATION ENGINE</button></a></div>"
